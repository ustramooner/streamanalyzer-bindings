 ...
 */
#ifndef STRIGI_PYTHONINDEXWRITER_H
#define STRIGI_PYTHONINDEXWRITER_H
...
public:
     ...
     * analyzed.
     */
    virtual void startAnalysis(const AnalysisResult*){}
     ...
     * being analyzed.
     */
    virtual void finishAnalysis(const AnalysisResult* r){}
    /**
...
public:
    ...
    virtual void addValue(const AnalysisResult* r, const RegisteredField* field,
      ...
      try{
        addValue(r, field, (PyObject*)SWIG_From_std_string(value)); 
	    }_CLCATCH();
    }
    /**
     * @brief Add a field to the index.
    ...
    virtual void addValue(const AnalysisResult* r, const RegisteredField* field,
        int32_t value){
      try{
        addValue(r, field, (PyObject*)SWIG_From_int(value));
	    }_CLCATCH();
    }
    /**
     * @brief Add a field to the index.
    ...
    virtual void addValue(const AnalysisResult* r, const RegisteredField* field,
        uint32_t value){
      try{
        addValue(r, field,  (PyObject*)SWIG_From_unsigned_SS_long(value));
	    }_CLCATCH();
    }
    /**
     * @brief Add a field to the index.
    ...
    virtual void addValue(const AnalysisResult* r, const RegisteredField* field,
        double value){
      try{
        addValue(r, field, (PyObject*)PyFloat_FromDouble(value)); 
	    }_CLCATCH();  
    }
    /**
     * @brief Add a field to the index.
    ...
    virtual void addValue(const AnalysisResult* r, const RegisteredField* field,
      ...
      try{
        addValue(r, field, (PyObject*)Py_BuildValue("{s#:s#}", name.c_str(), name.length(), value.c_str(), value.length()));
	    }_CLCATCH();
    }

    void addText(const AnalysisResult* r, const char* text, int32_t length){
      if ( length > 0 ){
        try{
          addText(r, std::string(text, length));
	      }_CLCATCH();
      }
    }
     ...
     **/
    virtual void deleteEntries(const std::vector<std::string>& entries){ }
    /**
